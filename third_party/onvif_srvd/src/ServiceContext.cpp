...
tt__AudioSourceConfiguration* StreamProfile::get_audio_src_cfg(struct soap *soap) const
{
    auto cfg = soap_new_tt__AudioSourceConfiguration(soap);
    if (cfg)
    {
        cfg->soap_default(soap);
        cfg->UseCount = 1;
        cfg->Name = name + " Audio Source";
        cfg->token = name + "_audio_source";
        cfg->SourceToken = cfg->token;
    }
    return cfg;
}
...
{
    auto cfg = soap_new_tt__AudioEncoderConfiguration(soap);
    if (cfg)
    {
        cfg->soap_default(soap);
        cfg->UseCount = 1;
        cfg->Name = name + " Audio Encoder";
        cfg->token = name + "_audio_encoder";
        cfg->Encoding = static_cast<tt__AudioEncoding>(audio_type);
        cfg->Bitrate = audio_bitrate;
        cfg->SampleRate = audio_rate;
        cfg->Multicast = soap_new_tt__MulticastConfiguration(soap);
        cfg->SessionTimeout = 60000000;
    }
    return cfg;
}
...
{
    auto source = soap_new_tt__AudioSource(soap);
    if (source)
    {
        source->soap_default(soap);
        source->token = name + "_audio_source";
        source->Channels = 1;
    }
    return source;
}
...
tt__Profile* StreamProfile::get_profile(struct soap *soap) const
{
    ...
    profile->AudioSourceConfiguration  = get_audio_src_cfg(soap);
...
bool StreamProfile::set_type(const char *new_val)
{
    ...
    else if( new_type == "H265" )
        type = static_cast<int>(tt__VideoEncoding::H265);
    else
    {
...
bool StreamProfile::set_audio_type(const char *new_val)
{
    std::string value(new_val ? new_val : "");
    if (value == "AAC") audio_type = static_cast<int>(tt__AudioEncoding::AAC);
    else if (value == "MP3") audio_type = static_cast<int>(tt__AudioEncoding::MP3);
    else { str_err = "audio type not supported"; return false; }
    return true;
}

bool StreamProfile::set_audio_rate(const char *new_val)
{
    audio_rate = new_val ? atoi(new_val) : 0;
    return audio_rate > 0;
}

bool StreamProfile::set_audio_bitrate(const char *new_val)
{
    audio_bitrate = new_val ? atoi(new_val) : 0;
    return audio_bitrate > 0;
}
...
void StreamProfile::clear()
{
    ...
    height = -1;
    type   = -1;
    audio_type = static_cast<int>(tt__AudioEncoding::AAC);
    audio_rate = 16;
    audio_bitrate = 32;
}
