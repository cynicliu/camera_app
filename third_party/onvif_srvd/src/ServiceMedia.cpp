    ...
    _trt__GetGuaranteedNumberOfVideoEncoderInstancesResponse &trt__GetGuaranteedNumberOfVideoEncoderInstancesResponse)
{
    ...
    for(auto& p : profiles)
    {
        ...
        else if (p.second.get_type() == static_cast<int>(tt__VideoEncoding::H265))
        {
            totalNumber += instancesNumber;
        }
    }
...
int MediaBindingService::GetAudioSources(
    _trt__GetAudioSources*, _trt__GetAudioSourcesResponse &response)
{
    auto ctx = (ServiceContext*)soap->user;
    for (auto& p : ctx->get_profiles())
        response.AudioSources.push_back(p.second.get_audio_src(soap));
    return SOAP_OK;
}
    ...
    _trt__GetAudioSourceConfigurationsResponse &response)
{
    auto ctx = (ServiceContext*)soap->user;
    for (auto& p : ctx->get_profiles())
        response.Configurations.push_back(p.second.get_audio_src_cfg(soap));
    return SOAP_OK;
}
    ...
    _trt__GetAudioEncoderConfigurationsResponse &response)
{
    auto ctx = (ServiceContext*)soap->user;
    for (auto& p : ctx->get_profiles())
        response.Configurations.push_back(p.second.get_audio_enc_cfg(soap));
    return SOAP_OK;
}
...
int MediaBindingService::GetAudioSourceConfiguration(
    _trt__GetAudioSourceConfiguration *request,
    _trt__GetAudioSourceConfigurationResponse &response)
{
    auto ctx = (ServiceContext*)soap->user;
    for (auto& p : ctx->get_profiles()) {
        auto cfg = p.second.get_audio_src_cfg(soap);
        if (cfg->token == request->ConfigurationToken) { response.Configuration = cfg; break; }
    }
    return SOAP_OK;
}

int MediaBindingService::GetAudioEncoderConfiguration(
    _trt__GetAudioEncoderConfiguration *request,
    _trt__GetAudioEncoderConfigurationResponse &response)
{
    auto ctx = (ServiceContext*)soap->user;
    for (auto& p : ctx->get_profiles()) {
        auto cfg = p.second.get_audio_enc_cfg(soap);
        if (cfg->token == request->ConfigurationToken) { response.Configuration = cfg; break; }
    }
    return SOAP_OK;
}
