...
static const char *help_str =
        ...
        "       --type         [value] Set Type for Profile Media Services (JPEG|MPEG4|H264|H265)\n"
        "       --audio_type   [value] Set Audio Type for Profile (AAC|MP3)\n"
        "       --audio_rate   [value] Set Audio sample rate in kHz\n"
        "       --audio_bitrate[value] Set Audio bitrate in kbps\n"
...
static const struct option long_opts[] =
{
    ...
    { "audio_type",    required_argument, NULL, LongOpts::audio_type   },
    { "audio_rate",    required_argument, NULL, LongOpts::audio_rate   },
    { "audio_bitrate", required_argument, NULL, LongOpts::audio_bitrate},
...
void processing_cmd(int argc, char *argv[])
{
    ...
    while( (opt = getopt_long(argc, argv, short_opts, long_opts, NULL)) != -1 )
    {
        switch( opt )
        {
            ...
            case LongOpts::audio_type:
                        if (!profile.set_audio_type(optarg))
                            daemon_error_exit("Can't set audio type: %s\n", profile.get_cstr_err());
                        break;

            case LongOpts::audio_rate:
                        if (!profile.set_audio_rate(optarg))
                            daemon_error_exit("Can't set audio rate\n");
                        break;

            case LongOpts::audio_bitrate:
                        if (!profile.set_audio_bitrate(optarg))
                            daemon_error_exit("Can't set audio bitrate\n");
                        break;
